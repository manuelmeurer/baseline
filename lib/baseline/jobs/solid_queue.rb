...
module Baseline::Jobs::SolidQueue
  class << self
    def queues
    ...
    def discard_all_jobs(jobs_relation)
      Jobs.new(jobs_relation).discard_all
    end
    ...
    private
      ...
      def find_solid_queue_job!(job_id, jobs_relation)
        find_solid_queue_job(job_id, jobs_relation) or raise Baseline::Jobs::Errors::JobNotFound.new(job_id, jobs_relation)
      end

      def find_solid_queue_job(job_id, jobs_relation)
        Jobs.new(jobs_relation).find_job(job_id)
      end
      ...
      def deserialize_and_proxy_solid_queue_job(solid_queue_job, job_status = nil)
        ...
        Baseline::Jobs::JobProxy.new(solid_queue_job.arguments).tap do |job|
      ...
      def filter_raw_data_arguments(raw_data)
        raw_data.deep_dup.tap do |filtered_raw_data|
          filtered_raw_data["arguments"]["arguments"] =
            Baseline::Jobs
              .job_arguments_filter
              .apply_to(filtered_raw_data.dig("arguments", "arguments"))
        end
      end

      def status_from_solid_queue_job(solid_queue_job)
        Jobs::STATUS_MAP.invert[solid_queue_job.status]
      end

      def execution_error_from_solid_queue_job(solid_queue_job)
        if solid_queue_job.failed?
          Baseline::Jobs::ExecutionError.new \
      ...
      def worker_from_solid_queue_process(process)
        Baseline::Jobs::Worker.new(**worker_attributes_from_solid_queue_process(process))
      end
  ...
  class Jobs
    ...
    private
      ...
      def order_executions(executions)
        case
        when solid_queue_status.scheduled?
          executions.ordered
        when recurring_task_id.present?
          executions.order(job_id: :desc)
        else
          executions.order(job_id: :asc)
        end
      end
      ...
      def internally_limited_count
        count_limit = Baseline::Jobs.internal_query_count_limit + 1
      ...
      def execution_class_by_status
        if recurring_task_id.present?
          ...
          raise Baseline::Jobs::Errors::QueryError, "Status not supported: #{jobs_relation.status}"
        end
      end
      ...
      def filter_executions_by_process_id(executions)
        ...
        if solid_queue_status.claimed?
          ...
          raise Baseline::Jobs::Errors::QueryError, "Filtering by worker id is not supported for status #{jobs_relation.status}"
        end
      end
