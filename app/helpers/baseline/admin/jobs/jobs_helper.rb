...
module Baseline::Admin::Jobs::JobsHelper
  def job_title(job)
    job.job_class_name
  end
  ...
  def failed_job_backtrace(job)
    if clean_backtrace? && Baseline::Jobs.backtrace_cleaner
      Baseline::Jobs.backtrace_cleaner.clean(job.last_execution_error.backtrace).join("\n")
    else
  ...
  def job_delayed?(job)
    job.scheduled_at.before?(Baseline::Jobs.scheduled_job_delay_threshold.ago)
  end
  ...
  private
    ...
    def as_renderable_hash(argument)
      if argument["_aj_globalid"]
        ...
        Baseline::Jobs.job_arguments_filter.apply_to(argument)
